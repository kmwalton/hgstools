"""Axis-Aligned Bounding Box, specially made for rectangular prism HGS grids"""
...
class AABBox:
    ...
    @classmethod
    def from_blockspec(cls, blockspec):
        """Build an `AABBox` from anything interpretable as one (a *blockspec*).

        This is the canonical interpretation of a *blockspec* used throughout
        `pyhgs`. Wherever a method documents a parameter as "anything
        interpretable as an `AABBox`", the value is passed through here.

        Parameters
        ----------
        blockspec : `AABBox`, str, or sequence of 6 numbers
            Specification of an axis-aligned block by its min/max bounds in the
            interleaved order ``x0 x1 y0 y1 z0 z1``. Accepts any of

            - an existing `AABBox` (returned as a copy);
            - a `str` with the six values separated by spaces, commas, and/or
              parentheses, e.g. ``"0 10 0 5 0 1"`` or ``"(0,10, 0,5, 0,1)"``;
              or
            - any iterable of six numbers in that same order, e.g. a `tuple`,
              `list`, or array ``(0, 10, 0, 5, 0, 1)``.
        """
        # 0. Pass an existing box straight through (as a copy). Note that an
        # AABBox iterates in *grouped* order, so it must be handled before the
        # generic-sequence branch below, which expects interleaved order.
        if isinstance(blockspec, cls):
            return cls(*blockspec._data)

        # 1. Parse the blockspec into a list of 6 floats, in the order
        # x0, x1, y0, y1, z0, z1
        if isinstance(blockspec, str):
            vals = list(map(float, re.sub("[(),]", " ", blockspec).split()))
        else:
            vals = list(map(float, blockspec))

        if len(vals) != 6:
            raise ValueError(
                f"blockspec must contain exactly 6 values, got {len(vals)}")
    ...
    def to_bounds(self):
        """Return min/max bounds as a float array in interleaved blockspec order.

        i.e. ``[x0, x1, y0, y1, z0, z1]`` -- the same ordering accepted by
        `from_blockspec`.
        """
        # _data is grouped [x0, y0, z0, x1, y1, z1]; reshape/transpose to
        # interleave: (2,3) -> [[x0,y0,z0],[x1,y1,z1]] -> .T -> ravel
        return self._data.reshape((2, 3)).T.ravel()
    ...
    def to_blockspec(self, precision=3):
        ...
        """
        fmt = f".{precision}f"
        return " ".join(f"{v:{fmt}}" for v in self.to_bounds())

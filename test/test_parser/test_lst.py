...
class Test_LST_Parser(unittest.TestCase):
    ...
    def test_name_equiv(self):
        ...
        p1 = LSTFile(DATDIR+'steady_flow_transient_transport')
        p0 = LSTFile(DATDIR+'steady_flow_transient_transporto.lst')

        self.assertEqual(p0._fnin, p1._fnin)

    def test_n_ts_read(self):

        p = LSTFile(DATDIR+'steady_flow_transient_transporto.lst')
        self.assertEqual(p.get_n_ts(),10)

        p = LSTFile(DATDIR+'sflow_ttransport_w0solv_erroro.lst')
        self.assertEqual(p.get_n_ts(),24)

        p = LSTFile(DATDIR+'fail')
    ...
    def test_fluid_balance_trans(self):
        ...
        p = LSTFile(DATDIR+'tflow_ttransport_pm_frac_well')
    ...
    def test_mass_storage(self):
        ...
        p = LSTFile(DATDIR+'tflow_ttransport_pm_frac_wello.lst')
        ms = p.get_mass_storage(18, 1)
        ...
        with self.assertRaises(RuntimeError,msg='invalid species requested'):
            p.get_mass_storage(18, 0)
        with self.assertRaises(RuntimeError,msg='invalid species requested'):
            p.get_mass_storage(18, 2)

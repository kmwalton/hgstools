...
class RFG:
    ...
    def _findParser(fn):
        ...
        if not retParser:
            print('Could not parse input file "{}":{}'.format(fn,str(errmsg)), file=sys.stderr)
            sys.exit(1)
    ...
    def spewPreamble(self, moreMessages='', fout=sys.stdout):
        ...
        s.append( '! Dump of grid and explicit fractures defined in {}'.format(self.fnin) )
        if self.nudgeTo:
            s.append( '! Fractures and gridlines "nudged" to {} increments.'.format(self.nudgeTo) )
        if self.pmRefs:
            s.append(
    ...
    def get_array_size_facts(self):
        ...
        try:
            ...
            for a in 'xyz':
                spac = \
                    numpy.fromiter(iter(map(float,self.fxnet.iterGridLines(a))),float)
...
def make_arg_parser():
    ...
    parser.add_argument(
        ...
        action='append',
        default=[],
        help='''Add in extra grid lines specified here, e.g., "x= .1, 250, 399.9 ; z=0.1,199.9"
        If an axis is not specified, add this grid line to each dimension.
        ''')

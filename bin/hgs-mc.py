...
class HGS_MCRunner():
    ...
    def __init__(self, copy_command, tc_command, cu_command, base_dir, stop,
            start=0,):
        ...
        Arguments:
            ...
            cu_command : str
                A string representing the shell command and its arguments that
                will be used to cleanup the Hydrogeosphere
        ...
        self.cleanup_command = cu_command
    ...
    def _run_instance(
            d,
            copy_command, tc_command, cu_command,
            base_temp_dirn, isnam, iecnam, log_level,):
        ...
        Arguments:
            ...
            cu_command : str
                The cleanup command and arguments
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(
        epilog='''Note: if COPY_EXE or RUN_EXE are python script, or if any
        python scripts exist within the HGS toolchain, the will (should) be
        executed in an equivalent shell environment (i.e., same environment
    ...
    ap.add_argument( '--copy-command',
        ...
        type=str,
        metavar='"COPY_EXE [arg1 [arg2...]]" base_sim_dir mc_inst_dir',
        help='''The command line invocation of a script or a shell command that
        clones the base simulation inputs in a MC directory. If COPY_EXE
        requires arguments, then they all must be enclosed in quotes as one
        string. 'base_sim' and 'mc_inst_dir' are appended to COPY_EXE and its
        optional arguments.
        Default: "hgs-copy-inputs.py"''',
        )

    ap.add_argument( '--run-command',
        default='hgs-runall.py',
        type=str,
        metavar='"RUN_EXE [arg1 [arg2...]]"',
        help='''The name of the script or a shell command that that invokes
        the HGS toolchain and any postprocessing. This command and its arguments
        will be executed in the individual MC instance directories.
        Default: "hgs-runall.py"''',
        )

    ap.add_argument( '--cleanup-command',
        default='hgs-cleanup.py',
        type=str,
        metavar='"CLEANUP_EXE [arg1 [arg2...]]"',
        help='''The name of the script or a shell command that that invokes
        the HGS cleanup. This command and its arguments
        will be executed in the individual MC instance directories.
        Default: "hgs-cleanup.py"''',
        )
    ...
    args.copy_command = _fixup_user_command(args.copy_command)
    args.run_command = _fixup_user_command(args.run_command)
    args.cleanup_command = _fixup_user_command(args.cleanup_command)

    mc = HGS_MCRunner(args.copy_command, args.run_command, args.cleanup_command,
            args.base_sim, args.n_inst, args.start_at)

...
class HGS_MCRunner():
    ...
    def __init__(self, copy_command, tc_command, base_dir, keep_file, stop,
        ...
        Arguments:
            ...
            keep_file : str or None
                ...
                complete. This must be in a format readable by hgs-cleanup.py
        ...
        if keep_file:
            ...
            self.keep_file = str(Path(keep_file).absolute())
        else:
            logger.warning('No keep file specified. Most simulation outputs '\
                    'will be discarded')
    ...
    def _run_instance(d, copy_command, base_temp_dirn, tc_command,
            keep_file, isnam, iecnam, log_level,):
        ...
        Arguments:
            ...
            keep_file : Path-like
                Path-like object of the keep_file, or None
        ...
        runner = BaseRunner(None, simdir=d)
        runner.captureRunPreexisting()

        cp = subprocess.run(shlex.split(tc_command),cwd=d,)

        if cp.returncode == 0:
            # try to keep all input files generated by makeMesh
            runner.eraseRunOutputs(keep=keep_file)
